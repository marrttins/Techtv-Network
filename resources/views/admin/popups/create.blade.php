...
<div class="admin-form-container" style="max-width: 750px;">
    <form action="{{ url('admin/popups') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" class="input-field" value="{{ old('name') }}" placeholder="e.g. Special Tech Summit Discount" required>
        </div>

        <div class="form-group">
            <label class="form-label">Destination Link (Optional)</label>
            <input type="url" name="link" class="input-field" value="{{ old('link') }}" placeholder="e.g. https://techeconomy.ng/register">
        ...
        <div class="form-group">
            ...
            <select name="display_type" id="display_type" class="input-field" required>
                <option value="all_pages" {{ old('display_type', 'all_pages') == 'all_pages' ? 'selected' : '' }}>🌐 Show on All Pages across the site</option>
                <option value="specific_page" {{ old('display_type') == 'specific_page' ? 'selected' : '' }}>🎯 Show only on a specific page</option>
            </select>
        </div>

        {{-- Dynamic Page Selector --}}
        <div class="form-group" id="path-group" style="{{ old('display_type') == 'specific_page' ? '' : 'display: none;' }}">
            <label class="form-label">Select Target Page *</label>
            @php
                $currentPath = old('specific_page_path', '');
            @endphp
        ...
        <div class="form-group">
            ...
            <div style="margin-bottom: 1rem; background: #f8fafc; border: 1px solid var(--admin-border); border-radius: 8px; padding: 1rem; display: none;" id="preview-container">
        ...
        <div class="form-group" style="background: #f8fafc; border: 1px solid var(--admin-border); border-radius: 8px; padding: 1rem 1.25rem;">
            <label style="display: flex; align-items: center; gap: 0.65rem; cursor: pointer; font-weight: 700; color: #1e293b;">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', '1') == '1' ? 'checked' : '' }} style="width: 18px; height: 18px; accent-color: var(--accent);">
